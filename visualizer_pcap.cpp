...
public:
  ...
  void add_message(const std::string& text, bool is_buy, double price, uint32_t volume);
    ...
    case 103: { // Execute Order
      if (max_len >= 42) {
        ...
        if (g_visualizer) {
          ...
          // Determine if buy or sell based on price vs mid (heuristic)
          g_visualizer->add_message(msg, true, update.price, update.volume);
...
void apply_batched_updates() {
  ...
  if (!batch.empty()) {
    // Store updates for playback if stream is finished
    bool should_store = false;
    {
      std::lock_guard<std::mutex> lock(playback_mutex);
      should_store = (playback_index == 0 && playback_updates.empty());
    }
...
void parse_xdp_packet(const uint8_t *data, size_t length) {
  ...
  for (uint8_t i = 0; i < num_messages && offset < length; i++) {
    ...
    if (msg_size < 2 || offset + msg_size > length) break;
...
void OrderBookVisualizer::process_playback() {
  if (!is_playing || !stream_finished) return;
    ...
    switch (update.type) {
      ...
      case UpdateType::EXECUTE:
        ...
        // Add to message feed
        {
          char msg[256];
          snprintf(msg, sizeof(msg), "EXEC $%.2f x %u", update.price, update.volume);
          add_message(msg, true, update.price, update.volume);
        }
        // Add visual trade marker
...
void OrderBookVisualizer::render_message_feed() {
  ...
  for (const auto& entry : message_feed) {
    ImVec4 color = entry.is_buy ? ImVec4(0.0f, 1.0f, 0.0f, 1.0f) : ImVec4(1.0f, 0.0f, 0.0f, 1.0f);
...
void OrderBookVisualizer::add_message(const std::string& text, bool is_buy, double price, uint32_t volume) {
  std::lock_guard<std::mutex> lock(feed_mutex);
  MessageEntry entry;
  entry.text = text;
  entry.is_buy = is_buy;
...
void OrderBookVisualizer::render_order_book_graph() {
  ...
  double mid_price = stats.mid_price > 0 ? stats.mid_price : 
                      (best_bid > 0 && best_ask > 0 ? (best_bid + best_ask) / 2.0 : 
  ...
  // Build bid line (left side, green shaded region)
  std::vector<ImVec2> bid_points;
  bid_points.push_back(ImVec2(plot_pos.x, plot_pos.y + plot_size.y)); // Bottom left
  bid_points.push_back(ImVec2(bid_x, plot_pos.y + plot_size.y)); // Bottom at best bid
  
  // Add bid levels
  for (const auto &pair : bids) {
    double price = pair.first;
    uint32_t qty = pair.second;
    if (price < min_price || price > best_bid) continue;
    ...
    bid_points.push_back(ImVec2(x, y));
  }
  
  // Sort by x coordinate for proper polygon
  std::sort(bid_points.begin() + 2, bid_points.end(), 
            [](const ImVec2& a, const ImVec2& b) { return a.x < b.x; });
  
  bid_points.push_back(ImVec2(bid_x, plot_pos.y + plot_size.y)); // Back to bottom
  
  // Draw bid shaded region (green)
  if (bid_points.size() > 2) {
    ImU32 bid_fill = IM_COL32(0, 200, 0, 100);
    draw_list->AddConvexPolyFilled(bid_points.data(), (int)bid_points.size(), bid_fill);
    
    // Draw bid line
    for (size_t i = 1; i < bid_points.size() - 1; i++) {
      draw_list->AddLine(bid_points[i], bid_points[i+1], IM_COL32(0, 255, 0, 255), 2.0f);
    }
  }
  
  // Build ask line (right side, red shaded region)
  std::vector<ImVec2> ask_points;
  ask_points.push_back(ImVec2(ask_x, plot_pos.y + plot_size.y)); // Bottom at best ask
  ask_points.push_back(ImVec2(plot_pos.x + plot_size.x, plot_pos.y + plot_size.y)); // Bottom right
  
  // Add ask levels
  for (const auto &pair : asks) {
    double price = pair.first;
    uint32_t qty = pair.second;
    if (price < best_ask || price > max_price) continue;
    
    float x = plot_pos.x + (float)((price - min_price) / (max_price - min_price)) * plot_size.x;
    float y = plot_pos.y + plot_size.y - (float)(qty / (double)max_qty) * plot_size.y;
    ask_points.push_back(ImVec2(x, y));
  }
  
  // Sort by x coordinate for proper polygon
  std::sort(ask_points.begin() + 2, ask_points.end(), 
            [](const ImVec2& a, const ImVec2& b) { return a.x < b.x; });
  
  ask_points.insert(ask_points.begin() + 1, ImVec2(ask_x, plot_pos.y + plot_size.y)); // Back to bottom
  
  // Draw ask shaded region (red)
  if (ask_points.size() > 2) {
    ImU32 ask_fill = IM_COL32(200, 0, 0, 100);
    draw_list->AddConvexPolyFilled(ask_points.data(), (int)ask_points.size(), ask_fill);
    
    // Draw ask line
    for (size_t i = 1; i < ask_points.size() - 1; i++) {
      draw_list->AddLine(ask_points[i], ask_points[i+1], IM_COL32(255, 0, 0, 255), 2.0f);
    }
  }
  
  // Draw spread gap line
