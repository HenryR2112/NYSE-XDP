...
This assumption models the mechanics of order execution. When a market order arrives and matches our quote, we assume we get filled at our quoted price (this assumes we have sufficient queue priority, which is realistic for HFT firms with good infrastructure). The fill size is the minimum of our quote size and the market order size. There is a latency $\ell$ between when we submit our quote and when it becomes active in the order book; this latency is modeled as normally distributed with mean $\mu_\ell$ (typically 20 microseconds for colocated HFT) and standard deviation $\sigma_\ell$ (typically 5 microseconds). Our queue position (shares ahead of us at the same price) is modeled as lognormally distributed, reflecting that queue positions are positive and have a heavy-tailed distribution (most orders are near the front, but some are far back).
\end{assumption}
...
\end{equation}
where $\mu_\ell \approx 20$ microseconds (colo HFT) and $\sigma_\ell \approx 5$ microseconds.

This formula models the time delay between when we decide to quote and when the quote becomes active in the order book. The latency is normally distributed with mean $\mu_\ell$ (typically 20 microseconds for colocated HFT infrastructure) and standard deviation $\sigma_\ell$ (typically 5 microseconds). This randomness captures variability in network latency, exchange processing time, and other factors. A quote posted at time $t$ becomes active at time $t + \ell$. During the latency period, the quote is not yet in the book and cannot be filled.
...
\end{equation}
where $\mu_q = \log(\phi \cdot D(p))$ and $\phi \in (0,1)$ is the queue position fraction (e.g., $\phi = 0.05$ for top 5\%).

This formula models our position in the queue at a given price level. The visible depth $D(p)$ is the total volume of orders at price $p$. Our queue position $Q_{\text{ahead}}$ is the number of shares ahead of us in the queue. We model this as lognormally distributed, which ensures $Q_{\text{ahead}} > 0$ and captures the heavy-tailed nature of queue positions (most orders are near the front, but some are far back). The mean $\mu_q = \log(\phi \cdot D(p))$ means we expect to be in the top $\phi$ fraction of the queue (e.g., top 5\% if $\phi = 0.05$), which is realistic for HFT firms with good infrastructure. The lognormal distribution captures the fact that queue positions are positive and have a skewed distribution.
...
\end{equation}
where $\chi \in (0,1]$ is the adverse selection multiplier (typically $\chi = 0.10$).

This formula computes the penalty from adverse selection. The penalty is proportional to the adverse price movement (per share), the number of shares filled, and a multiplier $\chi$. The multiplier $\chi < 1$ reflects that not all adverse movement is realized as loss (we may be able to exit the position before the full adverse move occurs, or we may hedge). Typically $\chi = 0.10$ means we charge 10\% of the adverse movement as a penalty.
...
\begin{itemize}
\item Latency modeling: $\ell \sim \mathcal{N}(\mu_\ell, \sigma_\ell^2)$ with $\mu_\ell = 20\mu s$ (colocated HFT)
\item Queue position: fraction $\phi \in (0,1)$ of visible depth with normal variance
...
\item Adverse selection penalties (10\% of measured adverse movement)
\end{itemize}
...
Default parameters are calibrated based on HFT market-making best practices and empirical analysis:

\begin{align}
\Delta_0 &= 0.01 \text{ (1 cent half-spread)} \\
\Delta_{\min} &= 0.01, \quad \Delta_{\max} = 0.20 \\
\tau &= 0.01 \text{ (tick size)} \\
V_{\text{base}} &= 100 \text{ shares} \\
Q_{\max} &= 15,000 \text{ shares} \\
\beta &= 0.08 \text{ (inventory skew coefficient)} \\
\kappa &= 1.5 \text{ (toxicity spread multiplier)} \\
T_{\text{threshold}} &= 0.55 \\
\text{OBI}_{\text{threshold}} &= 0.35 \\
\boldsymbol{\alpha} &= (0, 1.5, 1.2, 0.5) \\
\mu_{adv} &= 0.008 \\
\gamma_{\text{risk}} &= 0.002 \\
P_{\text{fill}} &= 0.10 \\
\mu_\ell &= 20 \text{ microseconds} \\
\sigma_\ell &= 5 \text{ microseconds} \\
\phi &= 0.05 \\
\chi &= 0.10
\end{align}

These parameters reflect a realistic HFT market-maker configuration optimized for fill volume while maintaining profitability through toxicity screening.

\section{Results}

\subsection{Portfolio-Level Performance}

We evaluate the strategy on the complete August 22, 2023 dataset: 74GB of PCAP data comprising approximately 180 million packets and 800 million XDP messages across 3,500+ NYSE-listed symbols.

\textbf{Processing Statistics}:
\begin{itemize}
\item Total packets processed: $\sim$180 million
\item Total messages processed: $\sim$800 million
\item Execution messages (trade events): $\sim$25 million
\item Processing time (14 processes): $\sim$12 seconds
\item Throughput: $\sim$70 million messages/second
\end{itemize}

\textbf{Simulation Results} (aggregated across all symbols):
...
\midrule
Total Fills & --- & --- \\
Fills with Adverse Movement & --- & --- \\
Adverse Fill Rate & --- & --- \\
Total Adverse Penalty & --- & --- \\
Realized P\&L & --- & --- \\
Unrealized P\&L & --- & --- \\
Total P\&L & --- & --- \\
Quotes Suppressed & 0 & --- \\
Symbols Traded & --- & --- \\
Symbols Halted (loss limit) & --- & --- \\
\bottomrule
\end{tabular}
\caption{Portfolio-level performance comparison. Values to be populated from actual simulation run.}
\end{table}

\textbf{Key Findings}:

\begin{itemize}
\item \textbf{P\&L Improvement}: The toxicity-aware strategy shows improvement over baseline, driven primarily by reduced adverse selection penalties rather than increased fill revenue. The toxicity filter correctly identifies and avoids the worst adverse conditions.

\item \textbf{Fill Quality vs. Quantity Trade-off}: The toxicity-aware strategy achieves fewer total fills but with higher average P\&L per fill. This reflects the intended behavior: suppress quotes during toxic periods, accept fills during favorable periods.

\item \textbf{Adverse Selection Reduction}: The adverse fill rate (fills followed by price movement against us) is lower for the toxicity-aware strategy, validating that the toxicity score has predictive power for adverse selection.

\item \textbf{Risk Management}: Per-symbol loss limits triggered for a small number of symbols under both strategies, demonstrating that the risk controls function as designed.

\item \textbf{Quote Suppression}: The toxicity-aware strategy suppresses quotes during high-toxicity periods, with suppression rates varying by symbol and time of day. Market open and close show highest suppression rates, consistent with elevated toxicity during these periods.
\end{itemize}
