...
Evaluate whether real-time toxicity screening can improve market-making profitability on NYSE equity markets.
%
\textbf{Method.}
We extend the Avellaneda-Stoikov optimal control framework with an online SGD logistic model for order flow toxicity estimation, backtested on 74\,GB of NYSE XDP Integrated Feed data (1.27 billion messages, August 22, 2023) and compared against an unscreened baseline.
%
\textbf{Results.}
Naive market-making incurs \$16,342 in losses; the toxicity-aware strategy achieves +\$792 net P\&L, a \$17,134 improvement. Intraday Sharpe ratio improves from $-1.45$ (baseline) to 2.03 (screened, $p < 0.001$). Adverse selection decreases 71.9\% ($p = 0.00025$) and inventory variance decreases 85.6\% ($p = 0.00019$). The toxicity score has Spearman $\rho = 0.118$ with realized adverse selection ($p < 10^{-10}$), with cancellation rate alone accounting for $\rho = 0.116$. Symbol-level bootstrap across 3,731 active symbols confirms robustness (95\% CI: [\$747, \$837]).
%
\textbf{Conclusions.}
Loss avoidance (\$16,342) is the dominant mechanism, not alpha generation (\$792). Online SGD learning does not improve over static weight initialization---a negative result confirming the initialization is near-optimal. Only 47.2\% of individual symbols improve; portfolio-level profitability emerges from cross-sectional diversification. Results are robust across all tested parameter combinations but limited to a single trading day.
\end{abstract}
...
\item Queue position ahead of us: $Q_{ahead} \sim \mathcal{N}(\mu_q, \sigma_q^2)$ shares, truncated at zero
\end{enumerate}
\end{assumption}
...
\end{cases}
\end{equation}
where $T_{\text{threshold}} \in (0,1)$ and $\epsilon_{\text{min}} = 0.0005$ (\$0.05 per 100 shares) are tuning parameters.
...
\end{equation}
where $\phi = 0.005$ reflects elite HFT front-of-queue positioning (top 0.5\%) and $\sigma_q = 0.1$ controls variance relative to the mean queue position. The distribution is truncated at zero to ensure non-negative queue depth.
...
After a fill at time $t$, we measure adverse price movement over a lookforward window $[t, t+\tau]$ ($\tau = 250$ microseconds):
\begin{equation}
...
\end{itemize}
Quote updates are rate-limited to a configurable interval (default 10 $\mu$s) to model realistic HFT infrastructure constraints.
...
\item Adverse selection: lookforward measurement over configurable window (default 250$\mu$s)
\end{itemize}

\item \textbf{P\&L Accounting}: Tracks realized/unrealized P\&L using weighted-average cost basis, including maker rebates (\$0.0025/share), clearing fees (\$0.00008/share), and adverse selection penalties (3\% of measured adverse movement).

\item \textbf{Risk Management}: Enforces per-symbol position limits (50,000 shares), daily loss limits (\$5,000), and portfolio-wide kill switches (\$500,000).
\end{enumerate}
...
\item \textbf{Quote Decision Engine}: Rate-limited to 10$\mu$s intervals. Compute toxicity score, order book imbalance, and inventory skew. Generate quotes: $p^{bid} = P_t - \Delta_t(\hat{T}_t)/2 + \kappa_{inv}(\rho_t)$. Update virtual order state.
...
The build system uses \texttt{-O3 -march=native -flto} for aggressive optimization including auto-vectorization and link-time optimization, with IEEE 754 floating-point semantics preserved for reproducibility.
...
\epsilon_{\min} &= 0.0005 \text{ (minimum expected PnL to quote, \$0.05 per 100 shares)} \\
...
Given \$792 of total profit, the margin between viability and loss is narrow. We conduct one-at-a-time (OAT) sensitivity analysis across four key parameters, varying each across a realistic range while holding others at default values. This approach does not capture multi-parameter interactions; a full factorial or Latin hypercube design would be needed to characterize interaction effects. However, the single dominant parameter ($\chi$, accounting for 48\% profit variation versus $\leq$17\% for all others) suggests that interaction effects are likely second-order relative to the main effect of the adverse selection multiplier.
...
$\epsilon_{\min}$ & Minimum expected PnL threshold for quoting ($5 \times 10^{-4}$) \\
